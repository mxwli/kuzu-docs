...
```
The `DATE` type needs to be cast explicitly as in `CAST("2024-01-01", "DATE")`while `4.4`, which is of type `DOUBLE`, 
...
the type name. The `AS` keyword comes next, followed by an existing type in the system. This type
is useful when you want to create a new type that is more relevant to your business
logic, or when you want to create an alias for an existing type.

The grammar for creating a user-defined type is as follows:

```cypher
CREATE TYPE <type_name> AS <existing_type>
```

Read more about user-defined types by clicking on the card below.

<LinkCard
  title="User-defined types"
  href="/cypher/data-types/user-defined-types"
  description="How to define and use a user-defined type"
/>
